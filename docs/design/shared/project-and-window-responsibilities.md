...
图像比较、鲁棒性、感知指纹、位平面、LSB、卷积和小波等边界拆分登记函数。该拆分只改善组合根可读性，
...
当前 Standalone 用十个独立 Scope 承载十个真实 Document；卷积核实验台和小波实验室与其他实例一样复用 Module/DI，窗口关闭时显式 Dispose Document 后再释放 Scope。
