...
图像比较、鲁棒性、感知指纹、位平面、LSB、卷积、小波、频域滤波、频谱遮罩、周期噪声、SVD 和颜色迁移等边界拆分登记函数。该拆分只改善组合根可读性，
...
当前 Standalone 用十五个独立 Scope 承载十五个真实 Document；调色板与颜色迁移和卷积核实验台、小波实验室、频域滤波、频谱遮罩编辑器、周期噪声、奇异值分解重建及其他实例一样复用 Module/DI，窗口关闭时显式 Dispose Document 后再释放 Scope。
